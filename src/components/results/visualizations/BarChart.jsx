...
const BarChart = ({ items }) => {
  ...
  return (
    <div className="w-full" style={{ backgroundColor: currentTheme.colors.background, marginTop: '1vh' }}>
      ...
      {metrics.map(metric => (
        <div key={metric} className=" w-full items-center">
          <h3 className="w-36 text-md font-medium mt-2" style={{ color: currentTheme.colors.text }}>{metric}</h3>
            ...
            {items.map((item, i) => {
              ...
              return (
                <div key={item.id} className="flex-1">
                  <div className="h-2 bg-gray-800 rounded-full overflow-hidden" style={{ backgroundColor: 'rgba(204, 204, 204, 0.56)' }}>
                    <div
