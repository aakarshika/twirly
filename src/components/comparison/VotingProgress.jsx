...
const VotingProgress = ({ isPressing, progress, userVoted, itemId, handleVote, startVoting, cancelVoting, votedItemId }) => {
  return (
    <div className="relative">
          {/* Progress Fill */}
          <div 
            className={` inset-0 transition-all duration-100 ${
              votedItemId === itemId ? 'bg-amber-400/30' : 'bg-primary-400/30'
            }`}
            style={{ 
          ...
          {/* Progress Marker */}
          <div className=" top-0 right-0 w-1 h-full bg-primary-500/50" />
