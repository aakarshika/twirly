                ...
                {(showAllMetrics ? topMetrics : topMetrics.slice(0, 3)).map((metric) => (
                  <div key={metric.title} className="space-y-1">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-300 capitalize" style={{ color: currentTheme.colors.text }}>{metric.title}</span>
              ...
              {showReviews && (
                <div className="mt-1 space-y-2">
                  {reviews.map((review) => (
                    <div
                      key={review.id}
                      className="p-2 bg-white/5 rounded-sm space-y-1"
                    >
                      ...
                      <p className="text-sm text-gray-300" style={{ color: currentTheme.colors.text }}>{review.text}</p>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
