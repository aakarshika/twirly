...
const ReviewModal = () => {
  ...
  const averageMetrics = item?.averageMetrics || {};
  ...
  const handleSubmit = async (e) => {
    ...
    // Log the error if the review insertion fails
    if (reviewError) {
      console.error('Error inserting review:', reviewError); // Log the entire error object
      return; // Exit if there's an error
    }

    // Log the reviewData to check if it's null
    console.log('Inserted review data:', reviewData);

    // Check if reviewData is null
    if (!reviewData) {
      console.error('Review data is null, cannot proceed with metrics insertion.');
      return; // Exit if reviewData is null
    }

    // Insert the review metrics into the review_metrics table
    const metricsToInsert = Object.entries(metrics).map(([metricName, value]) => ({
      review_id: reviewData[0].id, // Use the ID of the newly created review
      metric_name: metricName,
      value: value,
    }));

    // Log the metrics to be inserted
    ...
    if (metricsError) {
      console.error('Error inserting review metrics:', metricsError); // Log the error message
      return; // Handle the error appropriately
    }
    console.log('Metrics inserted:', metricsData);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-6">
                {Object.entries(averageMetrics).map(([metricName, metricData]) => (
                  <div 
                    key={metricName} 
                    className="space-y-2"
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <span className="text-gray-300 capitalize">{metricName}</span>
                      <span className="text-sm text-gray-400">
                        {metricData.average.toFixed(2)}/5
                      </span>
                    </div>
                    <div className="flex gap-1">
                      {renderStars(metricName, metricData.average)}
                    </div>
                  </div>
                ))}
              </div>
