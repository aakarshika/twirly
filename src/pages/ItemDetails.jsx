...
const ItemDetails = () => {
  ...
  // Function to fetch and update reviews for an item
  const fetchItemReviews = async (itemId) => {
    try {
      const { reviews } = await getItemReviews(itemId, 1, 3);
      
      // Update the items state with new reviews
      setItems(prevItems => 
        prevItems.map(item => {
          if (item.id === itemId) {
            return {
  ...
  // Function to refresh all items' reviews
  const refreshAllReviews = async () => {
    try {
      const updatedItems = await Promise.all(items.map(async item => {
        const { reviews } = await getItemReviews(item.id, 1, 3);
        return {
          ...item,
          reviews: reviews.map(review => ({
            id: review.id,
  ...
  // Function to fetch and update average metrics for an item
  const fetchItemAverageMetrics = async (itemId) => {
    try {
      const metrics = await getItemAverageMetrics(itemId);
      
      // Update the items state with new metrics
      setItems(prevItems => 
        prevItems.map(item => {
          if (item.id === itemId) {
            return {
  ...
  // Function to refresh all items' metrics
  const refreshAllMetrics = async () => {
    try {
      const updatedItems = await Promise.all(items.map(async item => {
        const metrics = await getItemAverageMetrics(item.id);
        return {
          ...item,
          averageMetrics: metrics
        };
      ...
      try {
        ...
        const items = await Promise.all(data.comparison_set_items?.map(async setItem => {
          try {
            ...
            // Get average metrics for this item
  ...
  // Add effect to refresh reviews when activeReviewItem changes
  useEffect(() => {
    if (activeReviewItem === null) {
      // When review modal is closed, refresh all reviews
      refreshAllReviews();
    }
  }, [activeReviewItem]);

  // Add effect to refresh metrics when activeReviewItem changes
  useEffect(() => {
    if (activeReviewItem === null) {
      // When review modal is closed, refresh all metrics
      refreshAllMetrics();
    }
  }, [activeReviewItem]);
